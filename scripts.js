$(document).ready(function () {
  ...
  // Make projects display as a grid with two items per row
  function setupProjectGrid() {
    // Add project-container class to the projects-grid div
    $(".projects-grid").addClass("project-container");
  }

  // Call the function to set up project grid
  setupProjectGrid();
  ...
  $(document).ready(function () {
    const scrollToTopBtn = $(
      '<button id="back-to-top" aria-label="滚动到顶部">' +
...
Type any command to execute.`,
    ...
    cowsay: (message = "Mooooo!") => {
      ...
      const fullMessage =
        message.toString() +
        (arguments.length > 1
          ? " " + Array.from(arguments).slice(1).join(" ")
          : "");
      return `
